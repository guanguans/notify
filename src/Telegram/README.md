# Telegram | [Home](./../../)

## [Usage example](./../../tests/Telegram/ClientTest.php)

## Related links

* [https://core.telegram.org/bots/features#botfather](https://core.telegram.org/bots/features#botfather)
* [https://core.telegram.org/bots/api#authorizing-your-bot](https://core.telegram.org/bots/api#authorizing-your-bot)
* [https://telegram.me/botfather](https://telegram.me/botfather)
* [https://core.telegram.org/bots/api#getupdates](https://core.telegram.org/bots/api#getupdates)
* [https://api.telegram.org/botxxx:yyy/getUpdates](https://api.telegram.org/botxxx:yyy/getUpdates)
* [https://core.telegram.org/bots/api#sendmessage](https://core.telegram.org/bots/api#sendmessage)
* [https://api.telegram.org/botxxx:yyy/sendMessage?chat_id=xx&text=text](https://api.telegram.org/botxxx:yyy/sendMessage?chat_id=xx&text=text)
